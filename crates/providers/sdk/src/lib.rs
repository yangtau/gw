...
/// `{ command }` array (Cursor `hooks.json`), not Claude's nested
/// `{ hooks: [{ type, command }] }` shape.
pub fn flat_command_hook_patch(provider: &str, event: &str) -> Patch {
    Patch {
        pointer: format!("/hooks/{event}"),
        mode: PatchMode::Ensure,
        value: json!({ "command": format!("gw hook {provider}") }),
    }
}
...
mod tests {
    ...
    fn flat_command_hook_patch_uses_cursor_entry_shape() {
        let patch = flat_command_hook_patch("cursor", "sessionStart");
